...
def handle_msg_posts():
    ...
    try:
        if msg_type == "send-msg":
            ...
            general_socket.send(pickle.dumps(
                reqBody | {"username": username, "size_of_file": getfile_size(msg)}))
...
def connect_to_servers():
    if os.environ.get('WERKZEUG_RUN_MAIN') == 'true':  # check if it's the main process
        ...
        while True:
            ...
            # meet_id = "abc"
            general_socket.send(pickle.dumps(
                {"email": email, "username": username, "meet_id": meet_id}))
            res = general_socket.recv(4)
            if res == b'0':
        ...
        stream = audio.open(format=AUDIO_FORMAT, channels=CHANNELS, rate=RATE,
                            input=True, output=True, frames_per_buffer=AUDIO_FRAME_RATE)
