...
def accept_video_connections():
    while True:
        ...
        recv_video_thread = threading.Thread(
            target=recv_video, args=(new_conn, addr))
...
def accept_audio_connections():
    while True:
        ...
        recv_audio_thread = threading.Thread(
            target=recv_audio, args=(new_conn, addr))
...
def accept_general_connections():
    while True:
        ...
        general_recv_thread = threading.Thread(
            target=general_recv, args=(new_conn, addr))
            ...
            if data:
                ...
                if msg_type == "send-file":
                    ...
                    while file_size > 0:
                        ...
                        print("Recieved, len", getsizeof(
                            file_data), len(file_data), count)
                        count += 1
