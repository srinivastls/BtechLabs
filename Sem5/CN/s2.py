...
def receive_and_send_chunks(sender_conn, receiver_conn, filename, final_len):
    target = clients_recv[receiver_conn-1]
    target.send(filename.encode(FORMAT))
    target.send(final_len.encode(FORMAT))
    time.sleep(3)
    while True:
...
def handle_client_file(conn, addr):
    ...
    while connected:
        try:
            ...
            final_len = conn.recv(SIZE).decode(FORMAT)
            time.sleep(2)
            print(f"Hello{final_len}")
            # Modify to specify the receiver client here
            receive_and_send_chunks(conn, rec_input, filename, final_len)
            print("123")
...
def all_Connect():
    while True:
        ...
        client_handler_video = threading.Thread(
            target=handle_client_video, args=(client_socket,))
        ...
        thread_chat = threading.Thread(
            target=handle_client_msg, args=(conn, addr))
        thread_chat.start()
        ...
        thread_audio = threading.Thread(
            target=handle_client_audio, args=(conn,))
        thread_audio.start()
...
def Send_thread():
    while True:
        ...
        thread_file = threading.Thread(target=handle_client_file, args=(
            conn, addr))  # calling the handle clinet funciton
        thread_file.start()
